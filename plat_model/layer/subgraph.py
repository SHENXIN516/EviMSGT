...
from plat_model.transform_AK.transform import SubgraphsTransform
from plat_model.transform_AK.config import cfg, update_cfg

from plat_model.transform_AK.element import MLP, DiscreteEncoder, Identity, VNUpdate
...
class subgraph(nn.Module):
    def __init__(self, model, dim_gnn):
        ...
        self.hop_embedding = nn.Linear(272, dim_gnn, bias=False)

        self.GAT_conv = model

        self.hop_embedder = nn.Embedding(20, 16)

        self.subgraph_transform = MLP(256, 256, nlayer=1, with_final_activation=True)
        self.context_transform = MLP(256, 256, nlayer=1, with_final_activation=True)
        self.gate_mapper_subgraph = nn.Sequential(nn.Linear(16, 256), nn.Sigmoid())
        self.gate_mapper_context = nn.Sequential(nn.Linear(16, 256), nn.Sigmoid())
        self.gate_mapper_centroid = nn.Sequential(nn.Linear(16, 256), nn.Sigmoid())
        self.pooling = 'mean'
        self.embs_combine_mode = 'concat'
        self.out_encoder = MLP(256 if self.embs_combine_mode == 'add' else 256 * len(self.embs), 256, nlayer=1,
