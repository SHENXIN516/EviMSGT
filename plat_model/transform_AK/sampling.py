...
def random_sampling(subgraphs_nodes, rate=0.5, minimum_redundancy=0, num_nodes=None):
    ...
    while True:
        ...
        node_selected_times = torch.bincount(subgraphs_nodes[1][check_values_in_set(subgraphs_nodes[0], selected)],
                                             minlength=num_nodes)
        if node_selected_times.min() >= minimum_redundancy:
            break
    return selected, node_selected_times

def shortest_path_sampling(edge_index, subgraphs_nodes, stride=2, minimum_redundancy=0, random_init=False,
                           num_nodes=None):
    if num_nodes is None:
        num_nodes = subgraphs_nodes[1].max() + 1
...
def min_set_cover_sampling(edge_index, subgraphs_nodes_mask, random_init=False, minimum_redundancy=2):
    ...
    if random_init:
        ...
        selected = subgraphs_nodes_mask.sum(
            -1).argmax().item()  # choose the maximum subgraph size one to remove randomness
